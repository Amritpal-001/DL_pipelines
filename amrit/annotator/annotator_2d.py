...
def start_annotator_2D(dataset:Annotator_2D_Dataset , cmap = 'gist_gray' ,
    ...
    print("Work best in Jupyter notebook")
...
def start_batch_annotator_2D(dataset:Annotator_2D_Dataset , rows = 2 ,  cols = 2 , cmap = 'gist_gray' ,
                 labels_dic = None , start_point = None):

    def _get_image_row(image_list , row_length , text_list = None , cmap = 'gist_gray'):
        fig = plt.figure()
        for index in range(row_length):
            plt.subplot(1,row_length , index)
            
            plt.tight_layout()
            plt.imshow(image_list[index], cmap = cmap)
            plt.title(text_list[index])
        
        plt.show()
        #print('done')

    def _get_drop_down_row(tag_list , row_length ):
        print('done')

    def _next_button(list_of_lists , row_length ):
        print('done')

    def _prev_button(list_of_lists , row_length ):
        print('done')
    ...
    for current_index in range(start_point , len(dataset)):
        print("Scan index" , current_index , '/' , dataset.df.shape[0])
        sample = dataset[current_index:current_index+cols]
